...
function CaptchaMini(params = {}) {
  const middleParams = Object.assign(
    {
      lineWidth: 0.5, // 线条宽度
      lineNum: 2, // 线条数量
      dotR: 1, // 点的半径
...
CaptchaMini.prototype.line = function () {
  for (let i = 0; i < this.lineNum; i++) {
    ...
    const endX = this.getRandom(0, this.canvas.width)
    const endY = this.getRandom(0, this.canvas.height)

    this.paint.beginPath()
    this.paint.lineWidth = this.lineWidth

    /* 获取颜色路径 */
    const colors = this.getColor(this.preGroundColor)
...
CaptchaMini.prototype.font = function () {
  ...
  for (let i = 0; i < this.length; i++) {
    ...
    this.paint[fontStyle](str[i], x, this.canvas.height / 2 + 4)
    this.paint.restore()
  }
}
...
CaptchaMini.prototype.draw = function (dom, callback = function () {}) {
  ...
  /* 随机画布颜色，使用背景色 */
  const colors = this.getColor(this.backGroundColor)
  this.paint.fillStyle =
    'rgba(' + colors[0] + ',' + colors[1] + ',' + colors[2] + ',' + '0.8)'
