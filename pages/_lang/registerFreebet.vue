  ...
  <div class="cl-accountRegister-wrap">
    ...
    <div class="cl-container">
      <div class="cl-matchBox">
        <div class="cl-freebetTitle"><span>FREEBET</span> FOR NEWBIES</div>
        <matchDetailInbox :meta="meta" :match_status="0" />
        <!-- 投注项 -->
        <div class="cl-betBox">
          <!-- 1X2 selectItem resultItem -->
          <div class="cl-flexcc cl-market">
            <template v-if="meta.markets">
              <div
                class="cl-flexcb cl-item"
                :class="[
                  selectOutcome && selectOutcome.id === item.id
                    ? 'cl-oddsButton'
                    : 'cl-noOddsButton'
                ]"
                v-for="(item, index) in meta.markets.outcomes"
                :key="index"
                @click="selectItem(meta.markets, item)"
              >
                <div class="cl-left">{{ item.display }}</div>
          ...
          <!-- freebet 投注金额 -->
          <div class="cl-flexcb cl-amountBox">
            <div class="cl-left">Free Stake: {{ stake }} XAF</div>
            <div class="cl-right">
              Payout:
              <span
                >{{
                  selectOutcome
                    ? (stake * (selectOutcome.odds - 1)).toFixed(2)
                    : '--'
                }}
                XAF</span
              >
...
export default {
  components: { headBack, accountModal, captcha, matchDetailInbox },
  ...
  computed: {
    ...
    isButtonCanNotUseComputed() {
      return (
        this.isButtonCanNotUse ||
        this.inputStatus.includes(2) ||
        !this.selectOutcome
      )
    },
    locale() {
  ...
  methods: {
    ...
    goRegister() {
      ...
      if (arr.length === 0 && this.selectOutcome) {
        // 是否需要验证码验证
    ...
    register() {
      ...
      this.$axios_api
        .register({
          ...
          type,
          device: this.imei,
          inviter: this.inviterCode || '',
          reg_cate: 1,
          is_active: 1, // 是否注册赠送freebet
    ...
    requestFreebet() {
      ...
      this.$axios_api
        ...
        .then((res) => {
          if (res.code === 0) {
            this.$store.commit('changeLoadingStatus', false)
            const num = Math.floor(Math.random() * res.data.result.length)
            this.meta = res.data.result[num]
          }
        })
        .catch(() => {
          this.$store.commit('changeLoadingStatus', false)
    ...
    selectItem(market, item) {
      ...
      this.$set(this.selectOutcome, 'match_id', this.meta.match_id)
      this.$set(this.selectOutcome, 'marketId', market.id)
      this.$set(this.selectOutcome, 'marketName', market.display)
    },
    // 投注
    freebetBet(token, gift_id, uid, login) {
      Lib.common.event_statistics('reg_freeBet_place_now_click')
      const outcomes = [this.selectOutcome.id]
      this.$axios_api
        .freebetBet({
          token,
          gift_id,
        ...
        .then((res) => {
          this.$store.commit('changeLoadingStatus', false)
          if (res.code === 0) {
            Lib.common.recFilFreeBet('record', [this.selectOutcome.match_id])
            Lib.common.event_statistics('reg_freebet_placeBet_success')
            Lib.common.errorReminder(
              this,
              '',
              '',
              this.$t('registerFreebet.success'),
              this.$t('registerFreebet.successArt'),
              this.$t('commonModal.yes'),
              this.$t('commonModal.no'),
              () => {
                // yes 按钮
                this.$router.replace({
                  path: `/${this.locale}/mybetDetail/${res.data.ticket_id}`
                })
              },
              () => {
                // no 按钮
                // 跳转到主页
                this.goHall(token, uid, login)
              }
            )
          } else {
            Lib.common.errorReminder(
              ...
              '',
              '',
              this.$t('registerFreebet.failed'),
              this.$t('registerFreebet.failedArt'),
              this.$t('commonModal.yes'),
              this.$t('commonModal.no'),
              () => {
                // yes 按钮
                this.$router.replace({ path: `/${this.locale}/freebet` })
              },
              () => {
                // no 按钮
                // 跳转到主页
...
.cl-accountRegister-wrap {
  ...
  .cl-container {
    ...
    /deep/ .inbox .bottom .middle .cl-status {
    ...
    .cl-inputBox {
      ...
      box-shadow: 0 -3rem/108 5rem/108 0 #cecede;
      border-radius: 38rem/108 38rem/108 0 0;
